...
@tool(description="获取指定城市的天气,消息以字符串形式返回")
def get_weather(city:str) -> str:
    return f"城市{city}天气为晴天，气温26°C，空气湿度50%，南风一级，AQI21，最近6小时降雨概率低"
...
def fetch_external_data(user_id: str, month: str) -> str:
    ...
    try:
        ...
        logger.error(f"用户{user_id}在{month}月没有数据")
        return ""


@tool(description="无入参，无返回值，调用后触发中间键，自动为报告生成的场景注入动态上下文信息，为后续提示词切换提供上下文信息")
